...
func main() {
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		LeaderElectionID:       "76c3f13a.platform.io",
	...
	if subscriptionID == "" {
		setupLog.Error(fmt.Errorf("AZURE_SUBSCRIPTION_ID not set"), "missing required env var")
		os.Exit(1)
	}
	armClient, err := armclient.New(subscriptionID)
	if err != nil {
		setupLog.Error(err, "unable to create ARM client")
		os.Exit(1)
	}

	definitionSvc := policydefinition.NewService(armClient)
	if err = (&controller.AzurePolicyDefinitionReconciler{
